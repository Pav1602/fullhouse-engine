...
# ============================================================================
# PROBE INSTRUMENTATION  (skantbot7.7 Stage 1 — diagnostic copy of 7.6)
# This file is a DIAGNOSTIC copy of bots/skantbot7.6/bot.py. The only changes
# vs 7.6 are read-only logging hooks below; NO decision logic is altered, so
# CRN determinism and per-opponent results are bit-identical to 7.6.
# Records one JSONL line per real decide() call (the hand_complete broadcast
# is skipped). Disabled unless SKANT_PROBE_DIR is set in the environment.
# ============================================================================
import os as _probe_os
import json as _probe_json

_PROBE_DIR = _probe_os.environ.get("SKANT_PROBE_DIR")
_PROBE_FH = None
_PROBE_TRACE = {}
_PROBE_HAND_IDS = set()
_PROBE_MY_ID = None
_PROBE_LAST_FINAL_STACK = INITIAL_STACK
# per-hand accumulator (P&L attribution) — reset on each new hand_id
_PROBE_HAND = {"id": None, "eq_override_call": False,
               "face_open_call": False, "face_open_fold": False, "was_hu": False}


def _probe_fh():
    global _PROBE_FH
    if _PROBE_FH is None and _PROBE_DIR:
        mid = _probe_os.environ.get("SKANT_MATCH_ID", "nomid")
        safe_mid = "".join(c if (c.isalnum() or c in "_-") else "_" for c in mid)
        path = _probe_os.path.join(_PROBE_DIR, f"probe_{safe_mid}_{_probe_os.getpid()}.jsonl")
        _PROBE_FH = open(path, "a")
    return _PROBE_FH
...
def _probe_emit(record):
    ...
    try:
        fh = _probe_fh()
        if fh is not None:
            fh.write(_probe_json.dumps(record) + "\n")
            fh.flush()
    except Exception:
        pass


def _probe_hand_reset(hid):
    global _PROBE_HAND
    _PROBE_HAND = {"id": hid, "eq_override_call": False,
                   "face_open_call": False, "face_open_fold": False, "was_hu": False}


def _probe_hand_summary(game_state):
    """Emit one per-hand P&L record from the hand_complete broadcast."""
    global _PROBE_LAST_FINAL_STACK
    if not _PROBE_DIR:
        return
    try:
        fs = game_state.get("final_stacks", {})
        if _PROBE_MY_ID is None or _PROBE_MY_ID not in fs:
            return
        cur = fs[_PROBE_MY_ID]
        delta = cur - _PROBE_LAST_FINAL_STACK
        _PROBE_LAST_FINAL_STACK = cur
        rec = {
            "type": "hand_summary",
            "match_id": _probe_os.environ.get("SKANT_MATCH_ID", ""),
            "hand_id": game_state.get("hand_id"),
            "chip_delta": delta,
            "eq_override_call": _PROBE_HAND["eq_override_call"],
            "face_open_call": _PROBE_HAND["face_open_call"],
            "face_open_fold": _PROBE_HAND["face_open_fold"],
...
def decide_preflop_6max(state: dict, position: str, hand: str, cfg: Config,
    ...
    if scenario == "face_open":
        ...
        small_open = open_ratio < cfg.small_open_threshold_bb
        if small_open:  # Small open detected
        ...
        _PROBE_TRACE["pf_open_ratio"] = round(open_ratio, 3)
        _PROBE_TRACE["pf_small_open"] = bool(small_open)
        ...
        if eff_3bet > 0 and rng.random() < eff_3bet:
            ...
            _PROBE_TRACE["pf_branch"] = "3bet"
            return {"action": "raise", "amount": safe_raise_amount(state, target)}

        table_call_freq = lookup_freq(call_range, hand) * call_freq_modifier
        call_freq = table_call_freq
        co_override = False
        ...
        if call_freq > 0 and owed <= state["your_stack"] * call_thresh:
            _PROBE_TRACE["pf_branch"] = "call_equity_override" if co_override else "call_table"
            return {"action": "call"}
...
def decide_preflop_hu(state: dict, position: str, hand: str, cfg: Config,
    ...
    if aggressors == 1:
        ...
        table_call_freq = lookup_freq(HU_BB_CALL_FREQS, hand) * call_freq_modifier
        call_freq = table_call_freq
        co_override = False
        pot = state["pot"]
        owed = state["amount_owed"]
...
def _probe_record(game_state, position, hand, street, action):
    """Build + emit one JSONL probe record. Read-only; never raises."""
    global _PROBE_MY_ID
    try:
        me = game_state.get("seat_to_act")
        players = game_state.get("players", [])
        # stash own bot_id (needed to read final_stacks at hand_complete)
        for p in players:
            if p.get("seat") == me:
                _PROBE_MY_ID = p.get("bot_id")
                break
        active = [p for p in players if not p.get("is_folded")]
        n_active = len(active)
        # cold-start: hands_observed for active opponents
        obs = []
        unknown = False
        for p in active:
            if p.get("seat") == me:
                continue
            bid = p.get("bot_id")
            prof = OPPONENTS.get(bid) if bid else None
            if prof is not None:
                obs.append(prof.hands_observed)
                if prof.hands_observed < CONFIG.cold_start_threshold_hands:
                    unknown = True
            else:
                obs.append(0)
                unknown = True
        hid = game_state.get("hand_id")
        if hid is not None and hid not in _PROBE_HAND_IDS:
            _PROBE_HAND_IDS.add(hid)
        if hid != _PROBE_HAND["id"]:
            _probe_hand_reset(hid)
        # per-hand flags for P&L attribution
        if n_active == 2:
            _PROBE_HAND["was_hu"] = True
        if street == "preflop":
            br = _PROBE_TRACE.get("pf_branch")
            if br == "call_equity_override":
                _PROBE_HAND["eq_override_call"] = True
            if br in ("call_equity_override", "call_table"):
                _PROBE_HAND["face_open_call"] = True
            if preflop_scenario(game_state) == "face_open" and action.get("action") == "fold":
                _PROBE_HAND["face_open_fold"] = True
        rec = {
            "match_id": _probe_os.environ.get("SKANT_MATCH_ID", ""),
            "hand_id": hid,
            "hand_idx": len(_PROBE_HAND_IDS),
            "seat": me,
            "position": position,
            "street": street,
            "hand": hand,
            ...
            "min_hands_obs": min(obs) if obs else None,
            "any_unknown": unknown,
        }
        if street == "preflop":
            rec["pf_scenario"] = preflop_scenario(game_state)
            rec["pf_branch"] = _PROBE_TRACE.get("pf_branch")
            rec["pf_open_ratio"] = _PROBE_TRACE.get("pf_open_ratio")
            rec["pf_small_open"] = _PROBE_TRACE.get("pf_small_open")
        else:
...
def decide(game_state: dict) -> dict:
    ...
    global _EQUITY_CACHE, our_match_delta, _PROBE_TRACE
    _EQUITY_CACHE = {}
    _PROBE_TRACE = {}
    t0 = time.time()

    if game_state.get("type") == "hand_complete":
        _probe_hand_summary(game_state)
    ...
    try:
        ...
        if game_state.get("type") != "hand_complete":
            _probe_record(game_state, position, hand, street, action)
