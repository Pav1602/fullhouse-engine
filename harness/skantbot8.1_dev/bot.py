...
def load_config_from_env() -> Config:
    """Dev-bot shim: read SKANT_<FIELD> env vars and override Config defaults."""
    cfg = Config()
    ...
    for f in fields(cfg):
        ...
        try:
            ...
            pass
    log_env = __import__("os").environ.get("SKANT_LOG_PATH")
    if log_env: cfg.log_path = log_env
    return cfg
...
def decide_preflop_6max(state: dict, position: str, hand: str, cfg: Config,
    ...
    if scenario == "open":
        ...
        if eff_freq > 0 and rng.random() < eff_freq:
            ...
            branch_fired = 'decide_preflop_6max_1495'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        if can_check:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_preflop_6max_1497'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_6max_1498'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if scenario == "face_open":
        ...
        if eff_3bet > 0 and rng.random() < eff_3bet:
            ...
            _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
            branch_fired = 'decide_preflop_6max_1529'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if call_freq > 0 and owed <= state["your_stack"] * call_thresh:
            ...
            branch_fired = 'decide_preflop_6max_1543'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if can_check:
            ...
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_6max_1547'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if scenario == "face_3bet_as_raiser":
        ...
        if fourbet_risk >= cfg.fourbet_commit_threshold:
            ...
            if fivebet_freq > 0 or hand in TIGHT_MONSTERS:
                ...
                branch_fired = 'decide_preflop_6max_1567'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
            _action_to_return = {"action": "fold"}
            branch_fired = 'decide_preflop_6max_1568'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if stack_bb < cfg.shallow_jam_threshold_bb:
            ...
            if fivebet_freq > 0 or hand in TIGHT_MONSTERS:
                _action_to_return = {"action": "all_in"}
                branch_fired = 'decide_preflop_6max_1574'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
            _action_to_return = {"action": "fold"}
            branch_fired = 'decide_preflop_6max_1575'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if eff_4bet > 0 and rng.random() < eff_4bet:
            ...
            branch_fired = 'decide_preflop_6max_1584'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if hand in {"JJ", "TT", "AKo", "AQs"} and owed <= state["your_stack"] * call_thresh:
            _action_to_return = {"action": "call"}
            branch_fired = 'decide_preflop_6max_1598'
            _log_decision(state, branch_fired, locals())
            return _action_to_return

        if can_check:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_preflop_6max_1601'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_6max_1602'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if scenario == "face_3bet_cold":
        ...
        if hand in TIGHT_MONSTERS:
            _action_to_return = {"action": "all_in"}
            branch_fired = 'decide_preflop_6max_1610'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if hand in {"JJ", "AKo", "AQs"} and owed <= state["your_stack"] * call_thresh_cold:
            _action_to_return = {"action": "call"}
            branch_fired = 'decide_preflop_6max_1616'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        if can_check:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_preflop_6max_1618'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_6max_1619'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if scenario in ("face_4bet_as_raiser", "face_5bet_as_raiser"):
        ...
        if lookup_freq(FIVEBET_FREQS, hand) > 0 or hand in PURE_MONSTERS:
            _action_to_return = {"action": "all_in"}
            branch_fired = 'decide_preflop_6max_1626'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        if can_check:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_preflop_6max_1628'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_6max_1629'
        _log_decision(state, branch_fired, locals())
        return _action_to_return

    if can_check:
        _action_to_return = {"action": "check"}
        branch_fired = 'decide_preflop_6max_1632'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    _action_to_return = {"action": "fold"}
    branch_fired = 'decide_preflop_6max_1633'
    _log_decision(state, branch_fired, locals())
    return _action_to_return
...
def decide_preflop_hu(state: dict, position: str, hand: str, cfg: Config,
    ...
    if not passes_variance_check(state, state["amount_owed"], state["your_cards"], cfg):
        if can_check:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_preflop_hu_1643'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_hu_1644'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    aggressors = count_aggressors(state)
    my_raises = count_my_raises(state)
    ...
    if aggressors == 0:
        if position == "BTN":
            ...
            if eff_freq > 0 and rng.random() < eff_freq:
                ...
                _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
                branch_fired = 'decide_preflop_hu_1654'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
            if can_check:
                _action_to_return = {"action": "check"}
                branch_fired = 'decide_preflop_hu_1656'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
            _action_to_return = {"action": "fold"}
            branch_fired = 'decide_preflop_hu_1657'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        else:
            if can_check:
                _action_to_return = {"action": "check"}
                branch_fired = 'decide_preflop_hu_1660'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
            _action_to_return = {"action": "fold"}
            branch_fired = 'decide_preflop_hu_1661'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
    ...
    if aggressors == 1 and my_raises == 0:
        ...
        if call_freq > 0 and owed <= state["your_stack"] * cfg.threebet_call_threshold_pct:
            _action_to_return = {"action": "call"}
            branch_fired = 'decide_preflop_hu_1691'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        if can_check:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_preflop_hu_1693'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_hu_1694'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if lookup_freq(HU_BB_5BET_FREQS, hand) > 0 or hand in PURE_MONSTERS:
        _action_to_return = {"action": "all_in"}
        branch_fired = 'decide_preflop_hu_1708'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    _action_to_return = {"action": "fold"}
    branch_fired = 'decide_preflop_hu_1709'
    _log_decision(state, branch_fired, locals())
    return _action_to_return
...
def decide_river(state: dict, position: str, eq: float, opp_profile, cfg: Config, rng: random.Random) -> dict:
    ...
    if owed > 0:
        ...
        if eq >= required_eq:
            ...
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_river_1743'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if eq < 0.20:
        ...
        if rng.random() < base_bluff_freq:
            target = int(pot * 0.5)
            _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
            branch_fired = 'decide_river_1771'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
            
    # Middle of range -> Check
    _action_to_return = {"action": "check"}
    branch_fired = 'decide_river_1774'
    _log_decision(state, branch_fired, locals())
    return _action_to_return
...
def decide_postflop(state: dict, position: str, cfg: Config,
    ...
    if can_check:
        ...
        if eq >= cfg.equity_value_bet:
            ...
            _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
            branch_fired = 'decide_postflop_1847'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if was_pf_aggressor and street in ("turn", "river") and eq < cfg.equity_thin_value and not facing_station:
            ...
            if rng.random() < barrel_freq:
                ...
                _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
                branch_fired = 'decide_postflop_1886'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
        ...
        if eq >= cfg.equity_thin_value and in_position:
            target = int(state["current_bet"] + pot * cfg.sizing_thin)
            _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
            branch_fired = 'decide_postflop_1891'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if not was_pf_aggressor and not in_position and eq > cfg.oop_passive_value_threshold:
            if opp_profile is not None and opp_profile.aggression_factor < cfg.passive_aggression_threshold:
                ...
                _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
                branch_fired = 'decide_postflop_1897'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
        ...
        if facing_station:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_postflop_1901'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        _action_to_return = {"action": "check"}
        branch_fired = 'decide_postflop_1910'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if owed <= 0:
        _action_to_return = {"action": "check"}
        branch_fired = 'decide_postflop_1914'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if eq >= cfg.equity_raise_threshold and not facing_maniac:
        ...
        _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
        branch_fired = 'decide_postflop_1934'
        _log_decision(state, branch_fired, locals())
        return _action_to_return

    if eq >= required_eq:
        _action_to_return = {"action": "call"}
        branch_fired = 'decide_postflop_1937'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if eq >= (cfg.equity_value_bet - cfg.k_commit * commitment_factor) and variance_term <= 0:
        _action_to_return = {"action": "call"}
        branch_fired = 'decide_postflop_1944'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if eq >= (call_thresh - cfg.k_commit * commitment_factor) and owed <= pot * cfg.pot_odds_buffer_marginal and variance_term <= 0:
        _action_to_return = {"action": "call"}
        branch_fired = 'decide_postflop_1957'
        _log_decision(state, branch_fired, locals())
        return _action_to_return

    _action_to_return = {"action": "fold"}
    branch_fired = 'decide_postflop_1959'
    _log_decision(state, branch_fired, locals())
    return _action_to_return
