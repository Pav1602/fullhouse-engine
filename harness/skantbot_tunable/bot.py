"""
Thin importlib shim for bots/skantbot3/bot (1).py

This file exists to give skantbot3 a path without spaces, making it safe to
...
skantbot3 top-level code → CONFIG = load_config_from_env() reads the overrides.
...
_SKANTBOT3_PATH = str(
    Path(__file__).parent.parent.parent / "bots" / "skantbot3" / "bot (1).py"
)

_spec = importlib.util.spec_from_file_location("skantbot3_module", _SKANTBOT3_PATH)
_mod  = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_mod)

BOT_NAME   = getattr(_mod, "BOT_NAME",   "SkantBot3_Tunable")
