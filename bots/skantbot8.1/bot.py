...
class Config:
    ...
    rfi_tightness: float = 1.510276228508083
    threebet_tightness: float = 0.7285931462299398
    fourbet_tightness: float = 1.239842784178138
    ...
    stack_short_tightness: float = 0.70490561771993
    ...
    shrink_widening_factor: float = 0.07138964394733673
    ...
    equity_value_bet: float = 0.5634152954776855        # bet for value above this
    equity_thin_value: float = 0.4339038368639582       # thin value-bet IP only
    equity_call_threshold: float = 0.4340568549542513   # marginal call threshold
    equity_raise_threshold: float = 0.7687326130974848  # raise instead of just call
    pot_odds_buffer_normal: float = 0.085321101838868  # extra equity required vs pot odds
    pot_odds_buffer_marginal: float = 0.2552825921483993 # how much pot we'll call vs marginal eq
    ...
    stack_risk_high_eq_normal: float = 0.8361514651207129        # equity needed if high risk, normal opp
    stack_risk_high_eq_maniac: float = 0.6833234421890474        # equity needed if high risk, vs maniac
    stack_risk_med_eq_normal: float = 0.547722338481879
    ...
    fourbet_call_threshold_pct: float = 0.1296778323913867       # cap on calling 3-bets out-of-position
    threebet_call_threshold_pct: float = 0.1800389205243701      # cap on calling raises with weaker hands

    # --- C-bet (continuation bet) ---
    cbet_freq_base: float = 0.6372901014972446
    k_texture_paired: float = 0.3297955576391428
    k_texture_monotone: float = 0.2566143665269354
    k_texture_connected: float = 0.0442922333025374
    k_texture_high_card: float = -0.1454215608514187
    spr_commit_threshold: float = 2.5395855491674575
    spr_smoothness: float = 2.4380477162197742
    k_commit: float = 0.0073147270862829
    river_mdf_aggression: float = 1.178107156329292
    river_v2b_half_pot: float = 2.6227859843348043
    river_v2b_pot_sized: float = 1.0
    river_v2b_overbet: float = 0.5
    k_river_bluff_blocker: float = 0.17209477294167339
    k_standing: float = 0.213936197750987
    standing_alpha: float = 0.1320726431595448
    standing_beta: float = 0.3152461386625292
    river_value_thin_threshold: float = 0.614173709496598
    river_value_strong_threshold: float = 0.7502847317179724
    ...
    cbet_size_pct: float = 0.5765125671526302

    # --- Small open defense ---
    small_open_threshold_bb: float = 2.634259946328461
    small_open_3bet_boost: float = 1.994224240077366
    small_open_call_boost: float = 1.730311916184485
    ...
    cbet_multiway_penalty: float = 0.5739309354414985

    # --- Bluff frequencies ---
    bluff_freq_ip: float = 0.0226640573328052
    bluff_freq_oop: float = 0.0498240986843801
    fourbet_bluff_freq: float = 0.1064617535920478
    ...
    variance_c: float = 0.0373763363240367
    # --- Opponent modelling ---
    k_bluff_vs_cbet_folder: float = 0.1966541567811665
    k_bluff_vs_2barrel_folder: float = 0.3518289296900118
    k_bluff_vs_3barrel_folder: float = 0.4760606991184524
    k_bluff_vs_wtsd: float = 0.08122340194306613
    k_value_size_vs_station: float = 0.1277254550361659
    k_tightness_vs_3bet_freq: float = 0.08133873374923735
    k_call_threshold_vs_aggression: float = 0.3779996679505744
    k_4bet_vs_3bet_freq: float = 0.06624082073349774
    ...
    # --- Phase 2a re-raise-freq scaling (range-narrowing un-narrow knobs) ---
    # The Phase 2a clause in aggressor_likely_range un-narrows the opponent
    # range when the opp has shown a high re-raise frequency profile (handles
    # min_raiser-style wide-raisers). Hardcoded in 7.7-7.10; tunable from 7.11.
    phase2a_baseline: float = 0.15           # rwf below this triggers no broadening
    phase2a_denominator: float = 0.85        # higher = slower un-narrow per rwf excess

    # --- 7.11 gating: action-strength override of Phase 2a ---
    # When opp shows committed aggression (jam OR bloated multi-street pot),
    # their specific action dominates over their habitual-aggression profile.
    # Two signals can fire the override; ANY one skips Phase 2a un-narrowing:
    #   (a) Engine-labeled all-in by aggressor (most explicit)
    #   (b) Pot is large relative to INITIAL_STACK (multi-street commitment)
    # min_raiser min-raises slowly: by river the pot is ~1500-2000 vs the
    ...
    skb8_bet_to_mean_multiplier: float = 1.6652147351691389
    skb8_min_obs_for_signal: int = 31
    skb8_min_bets_obs_for_signal: int = 3
...
def _log_decision(state, branch_fired, local_vars):
    if not CONFIG.log_path: return
    try:
        pot = state.get("pot", 0)
        owed = state.get("amount_owed", 0)
        
        eq = local_vars.get("eq")
        v_range = local_vars.get("v_range", [])
        v_range_size = len(v_range) if isinstance(v_range, (list, set, dict)) else None
        v_range_sample = list(v_range)[:10] if isinstance(v_range, (list, set, dict)) else []
        pot_odds = local_vars.get("pot_odds")
        req_eq = local_vars.get("required_eq")
        
        if pot_odds is None and owed > 0:
            pot_odds = owed / max(pot + owed, 1)
            
        opp_id = local_vars.get("aggressor_id") or local_vars.get("target_id") or local_vars.get("agg_bid")
        opp_n = None
        opp_rwf = None
        
        if opp_id and hasattr(OPPONENTS.get(opp_id), 'hands_observed'):
            prf = OPPONENTS[opp_id]
            opp_n = prf.hands_observed
            rwf = getattr(prf, 'raised_when_faced_postflop', 0)
            fb = getattr(prf, 'faced_bet_postflop', 0)
            if fb > 0:
                opp_rwf = rwf / fb

        action = local_vars.get("_action_to_return")
        ...
        with open(CONFIG.log_path, "a") as f:
            f.write(json.dumps({
                ...
                "hole": [str(c) for c in state.get("your_cards", [])],
                "board": [str(c) for c in state.get("community_cards", [])],
                "pot": pot,
                "owed": owed,
                "stack": state.get("your_stack", 0),
                "eq": eq,
                "v_range_size": v_range_size,
                "v_range_sample": [str(x) for x in v_range_sample],
                "pot_odds": pot_odds,
                "required_eq": req_eq,
                "branch": branch_fired,
                "action": action,
                "match_delta": our_match_delta,
                "opp_id": opp_id,
                "opp_n_hands": opp_n,
                "opp_rwf": opp_rwf
            }) + "\n")
...
# === Limp / check-down ranges by position ===
# When count_aggressors == 0 postflop, the opp neither raised nor 3-bet preflop.
# Their range is the "limp" range — hands they would have called/checked with
# rather than opened. This is typically:
#   - small/medium pocket pairs that don't 3-bet (22-99)
#   - small Aces wanting cheap flop equity (A2s-A5s)
#   - suited connectors / one-gappers
#   - some weak suited broadways
# NOT in the limp range: premium pairs (these would raise), AK-AQ (these raise).
# Pre-7.11 the bot fell through to RFI_FREQS[agg_pos] which is an *opening*
# range — wrong base when opp didn't open.
LIMP_FREQS: Dict[str, Dict[str, float]] = {
    "LJ":  _expand_to_freq_dict("22-77,A2s-A5s,76s,87s,98s,T9s"),
    "HJ":  _expand_to_freq_dict("22-77,A2s-A5s,65s,76s,87s,98s,T9s,JTs"),
    "CO":  _expand_to_freq_dict("22-88,A2s-A8s,54s,65s,76s,87s,98s,T9s,JTs,KQs"),
    "BTN": _expand_to_freq_dict("22-99,A2s-A9s,54s,65s,76s,87s,98s,T9s,JTs,KTs-KQs,QTs,T9o,98o,87o"),
    "SB":  _expand_to_freq_dict("22-99,A2s-A8s,54s,65s,76s,87s,98s,T9s,JTs,KTs-KQs,QTs,J9o,T9o,98o"),
    # BB never limps voluntarily (gets to see flop free); no entry.
}
...
# Chart-size invariants: fail loudly at import if any range chart silently
# under-expands (e.g. a future edit re-introduces the LOW-HIGH hyphen bug
# fixed in 7.8). Numbers are the post-fix correct sizes minus a safety margin.
assert len(HU_BB_CALL_FREQS) > 80, f"HU_BB_CALL_FREQS underpopulated: {len(HU_BB_CALL_FREQS)}"
assert len(THREEBET_CALL_FREQS[("BB", "BTN")]) > 80, f"THREEBET_CALL_FREQS[BB,BTN]: {len(THREEBET_CALL_FREQS[('BB','BTN')])}"
assert len(THREEBET_CALL_FREQS[("BB", "CO")]) > 55, f"THREEBET_CALL_FREQS[BB,CO]: {len(THREEBET_CALL_FREQS[('BB','CO')])}"
assert len(THREEBET_CALL_FREQS[("BB", "HJ")]) > 40, f"THREEBET_CALL_FREQS[BB,HJ]: {len(THREEBET_CALL_FREQS[('BB','HJ')])}"
assert len(THREEBET_CALL_FREQS[("BB", "LJ")]) > 35, f"THREEBET_CALL_FREQS[BB,LJ]: {len(THREEBET_CALL_FREQS[('BB','LJ')])}"
...
def board_texture(board: List[str]) -> str:
    ...
    if gap >= 6 and max_suit < 2:
        # Check if low two cards are connected (A-X-Y with X-Y close)
        sorted_idxs = sorted(ranks)  # ranks already indexes
        low_two_gap = sorted_idxs[1] - sorted_idxs[0]
        if low_two_gap <= 2:
            return "medium"  # A-low-low with low-low connected
        return "dry"
    return "medium"
...
def get_hand_rng(state: dict) -> random.Random:
    # NOTE: previously read SKANT_MATCH_ID env var for harness CRN, but
    # `import os` is rejected by the tournament validator. `hand_id` already
    # contains the match prefix (see sandbox/match.py:147), so per-hand
    # uniqueness is preserved without env var access.
    hand_id = state.get("hand_id", "")
    ...
    for ev in events:
        ...
        if t == "street_start":
            ...
            if street == "preflop":
                ...
                # Re-raise frequency: facing a bet = last_bettor is set
...
def _preflop_action_log(state: dict) -> list:
    ...
    Preflop ends when all NON-FOLDED players have equal bets. Folded seats'
    partial blinds/calls are excluded from the equalization check — otherwise
    a folded SB sitting at 50 chips (or any folder whose voluntary bet never
    matched the raise) keeps the loop running into postflop streets and the
    function returns the entire hand log. That silently bypassed range
    narrowing in `aggressor_likely_range` for ~every 6-max hand with folds.
    """
    log = state.get("action_log", [])
    pf = []
...
def _hand_class_to_combos(hand_class: str, used: set) -> List[Tuple[str, str]]:
    """All card-combo pairs for a hand class or specific combo.

    Supported lengths:
      2 - pocket pair, e.g. "AA" (6 combos)
      3 - suited/offsuit class, e.g. "AKs" (4) or "AKo" (12)
      4 - specific 2-card combo, e.g. "AsKs" or "Qc4c" (1 combo). Used by the
          board-aware narrowing to distinguish "Q4s could-be-a-flush" from
          "Qc4c IS the clubs flush combo".
    """
    combos = []
    if len(hand_class) == 2:
        r = hand_class[0]
        ...
        c1, c2 = hand_class[:2], hand_class[2:]
        if c1 not in used and c2 not in used:
            combos.append((c1, c2))
    return combos
...
def _narrow_range(rng_dict: dict, strength: str, board: list = None) -> dict:
    """Narrow a range to a strength tier with board-aware additions.

    Premium-pair narrowing alone fails on DEFENDING ranges (which exclude
    AA-QQ since those are 3-bet pre, not flat) — the intersection is empty
    and the prior `return subset if subset else rng_dict` fallback returned
    the entire input range, silently bypassing narrowing.

    This version, when narrowing to strong/medium, also adds board-derived
    value hands (sets, top-pair-good-kicker) and, on 3+flush boards, emits
    the SPECIFIC flush combo (4-char form like "Qc4c") for each suited
    hand-class whose flush variant is still available. The hand-class is
    then removed from the subset to avoid double-counting the flush combo.
    """
    strong = {"AA", "KK", "QQ", "JJ", "AKs", "AKo"}
    medium = strong | {"TT", "99", "88", "AQs", "AQo", "AJs", "AJo", "KQs", "KQo",
                       "T9s", "98s", "87s", "76s"}

    # Board-aware narrowing (existing behaviour)
    ...
    if strength == "strong":
        ...
        return dict(rng_dict)

    # Board-derived value hands (sets, TPGK, made flushes).
    if board and len(board) >= 3:
        ...
        suit_counts = {s: suits.count(s) for s in set(suits)}
        max_suit = max(suit_counts.values())
        flush_suit = max(suit_counts, key=suit_counts.get) if max_suit >= 3 else None
        top_rank = max(c[0] for c in board)

        for hand, freq in rng_dict.items():
            # Sets: pocket pair matching a board rank
            if len(hand) == 2 and hand[0] == hand[1] and hand[0] in board_ranks:
                subset[hand] = max(subset.get(hand, 0.0), freq)
            # Top-pair-good-kicker: any (suited OR offsuit) hand-class with
            # top-board-rank + kicker >= T. Defending ranges contain many
            # offsuit broadways (ATo+, KTo+, QTo+, JTo) that are TPGK plays
            # on K/A/Q-high boards; excluding them under-models opponent's
            # value subset.
            ...
            # Made flushes: emit the specific flush combo, drop the hand-class.
            if flush_suit and len(hand) == 3 and hand.endswith("s"):
                h1, h2 = hand[0], hand[1]
                c1, c2 = h1 + flush_suit, h2 + flush_suit
                if c1 not in board_card_set and c2 not in board_card_set:
                    combo = c1 + c2
                    subset[combo] = max(subset.get(combo, 0.0), freq)
                    # Drop the hand-class to avoid double-counting the flush variant.
                    # TPGK above may have re-added it; the flush model is stronger
                    # and supersedes it here.
                    subset.pop(hand, None)

    if not subset:
        # Defensive last resort: top 10% of input range by freq.
...
def aggressor_likely_range(state: dict, agg_seat: int) -> dict:
    ...
    # Detect "blind-defender as postflop aggressor": agg_seat called (didn't
    # raise) preflop and now leads/check-raises postflop. Their range is the
    # BB/SB *defending* range vs the opener — NOT an RFI opening range. The
    # prior code's `RFI_FREQS.get(agg_pos, RFI_FREQS["LJ"])` fallback was
    # using LJ opening range as the base for a BB defender, which is wrong.
    agg_pf_action = None
    opener_seat = None
    for e in pf_log:
    ...
    opener_pos = get_opp_position(state, opener_seat) if opener_seat is not None else None

    if (agg_pos in ("BB", "SB")
        and agg_pf_action == "call"
        and (agg_pos, opener_pos) in THREEBET_CALL_FREQS):
        base_range = THREEBET_CALL_FREQS[(agg_pos, opener_pos)]
    # 7.11: No preflop aggression at all → opp limped/checked. Use LIMP_FREQS,
    # not an RFI opening range. Pre-7.11 the bot used RFI_FREQS[agg_pos] here,
    # which made limpers look like openers (much wider). Same bug family as
    # the 7.10 BB-defender fix.
    elif aggressors == 0 and agg_pf_action in ("call", "check") and agg_pos in LIMP_FREQS:
        base_range = LIMP_FREQS[agg_pos]
    # 3-bettor detection: opp's preflop action was a raise, and there were
    # 2+ preflop raises (opp re-raised after someone else opened). Use
    # THREEBET_FREQS[(opp_pos, opener_pos)] as base, NOT RFI fallback.
    # Symmetric to the BB-defender path. Closes the missing-3-bettor-range
    # bug found via _trace_options.py on bust_195_h0128.
    ...
    # 1. Determine base preflop range (original logic)
    elif aggressors == 1 and agg_pos in RFI_FREQS:
    ...
    # 7.11 commitment override: skip Phase 2a un-narrowing on COMMITTED action.
    # Two signals (any one fires the override):
    #   (a) Engine-labeled all-in
    #   (b) Pot is large relative to INITIAL_STACK (multi-street commitment).
    #       min_raiser's slow barrels stay below this threshold; barreling opps
    #       like the bust_014 case (pot 42K, threshold 10K) trip it.
    # NOTE: a bet-size signal was tried (opp_bet/pot_before >= 0.6) but fires
    # against min_raiser's min-raises too. Pot/stack ratio is the cleaner cut.
    if _aggressor_last_action_is_allin(state, agg_seat):
        return narrowed
    pot_now = state.get("pot", 0)
    if pot_now >= CONFIG.committed_pot_ratio * INITIAL_STACK:
        return narrowed

    # Phase 2a: scale narrowing by opponent's true re-raise frequency.
    # A 100%-raiser's raise signals nothing → keep base_range.
    # A normal raiser (≤baseline) → full narrowing preserved.
    ...
    excess = max(0.0, reraise_freq - CONFIG.phase2a_baseline)
    w = max(0.0, 1.0 - excess / CONFIG.phase2a_denominator)

    if w >= 0.999:
        return narrowed
    keys = set(narrowed) | set(base_range)
    return {h: narrowed.get(h, 0.0) * w + base_range.get(h, 0.0) * (1.0 - w)
            for h in keys}
...
def passes_variance_check(state: dict, owed: int, hole: list, cfg: Config) -> bool:
    ...
    pot = state["pot"]
    # callable_pot excludes the opponent's uncallable excess: when they bet more
    # than our stack, those chips are returned to them and we cannot win them.
...
def decide_preflop_6max(state: dict, position: str, hand: str, cfg: Config,
    ...
    if not passes_variance_check(state, owed, state["your_cards"], cfg):
        if can_check:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_preflop_6max_1441'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_6max_1442'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if scenario == "open":
        ...
        if eff_freq > 0 and rng.random() < eff_freq:
            ...
            _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
            branch_fired = 'decide_preflop_6max_1466'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        if can_check:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_preflop_6max_1468'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_6max_1469'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if scenario == "face_open":
        ...
        if eff_3bet > 0 and rng.random() < eff_3bet:
            ...
            _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
            branch_fired = 'decide_preflop_6max_1500'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if call_freq > 0 and owed <= state["your_stack"] * call_thresh:
            _action_to_return = {"action": "call"}
            branch_fired = 'decide_preflop_6max_1514'
            _log_decision(state, branch_fired, locals())
            return _action_to_return

        if can_check:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_preflop_6max_1517'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_6max_1518'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if scenario == "face_3bet_as_raiser":
        ...
        if fourbet_risk >= cfg.fourbet_commit_threshold:
            ...
            if fivebet_freq > 0 or hand in TIGHT_MONSTERS:
                _action_to_return = {"action": "all_in"}
                branch_fired = 'decide_preflop_6max_1538'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
            _action_to_return = {"action": "fold"}
            branch_fired = 'decide_preflop_6max_1539'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if stack_bb < cfg.shallow_jam_threshold_bb:
            ...
            if fivebet_freq > 0 or hand in TIGHT_MONSTERS:
                _action_to_return = {"action": "all_in"}
                branch_fired = 'decide_preflop_6max_1545'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
            _action_to_return = {"action": "fold"}
            branch_fired = 'decide_preflop_6max_1546'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if eff_4bet > 0 and rng.random() < eff_4bet:
            _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, proposed_4bet)}
            branch_fired = 'decide_preflop_6max_1555'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if not facing_maniac and ip:
            ...
            if bluff_freq > 0 and rng.random() < cfg.fourbet_bluff_freq:
                _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, proposed_4bet)}
                branch_fired = 'decide_preflop_6max_1561'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
        ...
        if hand in {"JJ", "TT", "AKo", "AQs"} and owed <= state["your_stack"] * call_thresh:
            _action_to_return = {"action": "call"}
            branch_fired = 'decide_preflop_6max_1569'
            _log_decision(state, branch_fired, locals())
            return _action_to_return

        if can_check:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_preflop_6max_1572'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_6max_1573'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if scenario == "face_3bet_cold":
        ...
        if hand in TIGHT_MONSTERS:
            _action_to_return = {"action": "all_in"}
            branch_fired = 'decide_preflop_6max_1581'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if hand in {"JJ", "AKo", "AQs"} and owed <= state["your_stack"] * call_thresh_cold:
            _action_to_return = {"action": "call"}
            branch_fired = 'decide_preflop_6max_1587'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        if can_check:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_preflop_6max_1589'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_6max_1590'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if scenario in ("face_4bet_as_raiser", "face_5bet_as_raiser"):
        ...
        if lookup_freq(FIVEBET_FREQS, hand) > 0 or hand in PURE_MONSTERS:
            _action_to_return = {"action": "all_in"}
            branch_fired = 'decide_preflop_6max_1597'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        if can_check:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_preflop_6max_1599'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_6max_1600'
        _log_decision(state, branch_fired, locals())
        return _action_to_return

    if can_check:
        _action_to_return = {"action": "check"}
        branch_fired = 'decide_preflop_6max_1603'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    _action_to_return = {"action": "fold"}
    branch_fired = 'decide_preflop_6max_1604'
    _log_decision(state, branch_fired, locals())
    return _action_to_return
...
def decide_preflop_hu(state: dict, position: str, hand: str, cfg: Config,
    ...
    if not passes_variance_check(state, state["amount_owed"], state["your_cards"], cfg):
        if can_check:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_preflop_hu_1614'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_hu_1615'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    aggressors = count_aggressors(state)
    my_raises = count_my_raises(state)
    ...
    if aggressors == 0:
        if position == "BTN":
            ...
            if eff_freq > 0 and rng.random() < eff_freq:
                ...
                _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
                branch_fired = 'decide_preflop_hu_1625'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
            if can_check:
                _action_to_return = {"action": "check"}
                branch_fired = 'decide_preflop_hu_1627'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
            _action_to_return = {"action": "fold"}
            branch_fired = 'decide_preflop_hu_1628'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        else:
            if can_check:
                _action_to_return = {"action": "check"}
                branch_fired = 'decide_preflop_hu_1631'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
            _action_to_return = {"action": "fold"}
            branch_fired = 'decide_preflop_hu_1632'
            _log_decision(state, branch_fired, locals())
            return _action_to_return

    # 7.13 HU FIX: aggressors==1 was overloaded — "BB defends vs open" AND
    # "SB-as-opener faces 3-bet" both hit this branch. Split by my_raises.
    if aggressors == 1 and my_raises == 0:
        # We're BB facing BTN open (unchanged behavior).
        current_bet = state["current_bet"]
        ...
        if eff_3bet > 0 and rng.random() < eff_3bet:
            ...
            _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
            branch_fired = 'decide_preflop_hu_1650'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if call_freq > 0 and owed <= state["your_stack"] * cfg.threebet_call_threshold_pct:
            _action_to_return = {"action": "call"}
            branch_fired = 'decide_preflop_hu_1662'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        if can_check:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_preflop_hu_1664'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_hu_1665'
        _log_decision(state, branch_fired, locals())
        return _action_to_return

    # 7.13 HU FIX: NEW branch — we opened from SB, BB 3-bet, now we decide.
    # Mirrors 6-max face_3bet_as_raiser shape. Pre-7.13 this fell into the
    # aggressors==1 BB-defends branch, which used HU_BB_CALL_FREQS — a chart
    # designed for BB-defending-vs-open, not SB-facing-3-bet. A4o has freq
    # 1.0 in HU_BB_CALL_FREQS, so the bot called A4o vs 3-bet OOP (bust 195).
    if aggressors == 1 and my_raises == 1:
        owed = state["amount_owed"]
        current = state["current_bet"]
        # SB acts last preflop in HU after BB's 3-bet — effectively IP this decision.
        proposed_4bet = int(current * cfg.fourbet_size_ip)
        chips_to_4bet = proposed_4bet - state["your_bet_this_street"]
        fourbet_risk = chips_to_4bet / max(state["your_stack"] + state["your_bet_this_street"], 1)

        if fourbet_risk >= cfg.fourbet_commit_threshold:
            if lookup_freq(HU_BB_5BET_FREQS, hand) > 0 or hand in TIGHT_MONSTERS:
                _action_to_return = {"action": "all_in"}
                branch_fired = 'decide_preflop_hu_sb_face3bet_commit_jam'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
            _action_to_return = {"action": "fold"}
            branch_fired = 'decide_preflop_hu_sb_face3bet_commit_fold'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if stack_bb < cfg.shallow_jam_threshold_bb:
            if lookup_freq(HU_BB_5BET_FREQS, hand) > 0 or hand in TIGHT_MONSTERS:
                _action_to_return = {"action": "all_in"}
                branch_fired = 'decide_preflop_hu_sb_face3bet_shallow_jam'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
            _action_to_return = {"action": "fold"}
            branch_fired = 'decide_preflop_hu_sb_face3bet_shallow_fold'
            _log_decision(state, branch_fired, locals())
            return _action_to_return

        value_freq = lookup_freq(HU_BTN_4BET_FREQS, hand)
        eff_4bet = _effective_freq(value_freq, "BTN", "fourbet", stack_bb, 2, cfg)
        if eff_4bet > 0 and rng.random() < eff_4bet:
            _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, proposed_4bet)}
            branch_fired = 'decide_preflop_hu_sb_face3bet_4bet'
            _log_decision(state, branch_fired, locals())
            return _action_to_return

        call_thresh = cfg.fourbet_call_threshold_pct
        if hand in {"QQ", "JJ", "AKo", "AQs"} and owed <= state["your_stack"] * call_thresh:
            _action_to_return = {"action": "call"}
            branch_fired = 'decide_preflop_hu_sb_face3bet_call'
            _log_decision(state, branch_fired, locals())
            return _action_to_return

        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_hu_sb_face3bet_fold'
        _log_decision(state, branch_fired, locals())
        return _action_to_return

    # 7.13 HU FIX: aggressors==2 was also overloaded — "BB-3bettor faces 4-bet"
    # AND "SB-opener faces 5-bet" both hit this branch. Both should be all-in
    # or fold; the old code sized a partial 5-bet via fourbet_size_ip which
    # left chips behind when opp jammed (bust_021).
    if aggressors == 2 and my_raises == 1:
        # We're BB — we 3-bet, opp 4-bet. Jam or fold.
        if lookup_freq(HU_BB_5BET_FREQS, hand) > 0 or hand in PURE_MONSTERS:
            _action_to_return = {"action": "all_in"}
            branch_fired = 'decide_preflop_hu_bb_face4bet_jam'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_hu_bb_face4bet_fold'
        _log_decision(state, branch_fired, locals())
        return _action_to_return

    if aggressors == 2 and my_raises == 2:
        # We're SB — we opened, BB 3-bet, we 4-bet, opp 5-bet. Jam or fold.
        if lookup_freq(HU_BB_5BET_FREQS, hand) > 0 or hand in PURE_MONSTERS:
            _action_to_return = {"action": "all_in"}
            branch_fired = 'decide_preflop_hu_sb_face5bet_jam'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_preflop_hu_sb_face5bet_fold'
        _log_decision(state, branch_fired, locals())
        return _action_to_return

    if lookup_freq(HU_BB_5BET_FREQS, hand) > 0 or hand in PURE_MONSTERS:
        _action_to_return = {"action": "all_in"}
        branch_fired = 'decide_preflop_hu_1679'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    _action_to_return = {"action": "fold"}
    branch_fired = 'decide_preflop_hu_1680'
    _log_decision(state, branch_fired, locals())
    return _action_to_return
...
def decide_river(state: dict, position: str, eq: float, opp_profile, cfg: Config, rng: random.Random) -> dict:
    ...
    if owed > 0:
        ...
        if eq >= required_eq:
            _action_to_return = {"action": "call"}
            branch_fired = 'decide_river_1713'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        _action_to_return = {"action": "fold"}
        branch_fired = 'decide_river_1714'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if eq < 0.20:
        ...
        if rng.random() < base_bluff_freq:
            target = int(pot * 0.5)
            _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
            branch_fired = 'decide_river_1742'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
            
    # Middle of range -> Check
    _action_to_return = {"action": "check"}
    branch_fired = 'decide_river_1745'
    _log_decision(state, branch_fired, locals())
    return _action_to_return
...
def decide_postflop(state: dict, position: str, cfg: Config,
    ...
    if can_check:
        ...
        if eq >= cfg.equity_value_bet:
            ...
            _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
            branch_fired = 'decide_postflop_1818'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if was_pf_aggressor and street == "flop":
            ...
            if rng.random() < cbet_freq:
                ...
                _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
                branch_fired = 'decide_postflop_1839'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
        ...
        if was_pf_aggressor and street in ("turn", "river") and eq < cfg.equity_thin_value and not facing_station:
            ...
            if rng.random() < barrel_freq:
                ...
                _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
                branch_fired = 'decide_postflop_1857'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
        ...
        if eq >= cfg.equity_thin_value and in_position:
            target = int(state["current_bet"] + pot * cfg.sizing_thin)
            _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
            branch_fired = 'decide_postflop_1862'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if not was_pf_aggressor and not in_position and eq > cfg.oop_passive_value_threshold:
            if opp_profile is not None and opp_profile.aggression_factor < cfg.passive_aggression_threshold:
                ...
                _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
                branch_fired = 'decide_postflop_1868'
                _log_decision(state, branch_fired, locals())
                return _action_to_return
        ...
        if facing_station:
            _action_to_return = {"action": "check"}
            branch_fired = 'decide_postflop_1872'
            _log_decision(state, branch_fired, locals())
            return _action_to_return
        ...
        if eq < cfg.equity_thin_value and rng.random() < bluff_freq:
            target = int(state["current_bet"] + pot * cfg.sizing_thin)
            _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
            branch_fired = 'decide_postflop_1879'
            _log_decision(state, branch_fired, locals())
            return _action_to_return

        _action_to_return = {"action": "check"}
        branch_fired = 'decide_postflop_1881'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if owed <= 0:
        _action_to_return = {"action": "check"}
        branch_fired = 'decide_postflop_1885'
        _log_decision(state, branch_fired, locals())
        return _action_to_return

    effective_owed = min(owed, stack)
    # callable_pot excludes the opponent's uncallable excess (see passes_variance_check).
    callable_pot = pot - (owed - effective_owed)
    pot_odds = effective_owed / (callable_pot + effective_owed) if (callable_pot + effective_owed) > 0 else 1.0
    risk_pct = stack_risked_pct(state, effective_owed)
    ...
    if eq >= cfg.equity_raise_threshold and not facing_maniac:
        ...
        _action_to_return = {"action": "raise", "amount": safe_raise_amount(state, target)}
        branch_fired = 'decide_postflop_1905'
        _log_decision(state, branch_fired, locals())
        return _action_to_return

    if eq >= required_eq:
        _action_to_return = {"action": "call"}
        branch_fired = 'decide_postflop_1908'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if eq >= (cfg.equity_value_bet - cfg.k_commit * commitment_factor) and variance_term <= 0:
        _action_to_return = {"action": "call"}
        branch_fired = 'decide_postflop_1915'
        _log_decision(state, branch_fired, locals())
        return _action_to_return
    ...
    if eq >= (call_thresh - cfg.k_commit * commitment_factor) and owed <= pot * cfg.pot_odds_buffer_marginal and variance_term <= 0:
        _action_to_return = {"action": "call"}
        branch_fired = 'decide_postflop_1928'
        _log_decision(state, branch_fired, locals())
        return _action_to_return

    _action_to_return = {"action": "fold"}
    branch_fired = 'decide_postflop_1930'
    _log_decision(state, branch_fired, locals())
    return _action_to_return
