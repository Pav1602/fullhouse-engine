...
def dump_decision_log():
    if DECISION_LOG:
        ...
        with open(f"decision_log_{uid}.jsonl", "w") as f:
            for row in DECISION_LOG:
                f.write(json.dumps(row) + "\n")
atexit.register(dump_decision_log)

our_match_delta = 0

def _log_decision(state, position, hand, street, eq, opp_id, opp_profile, 
                   branch, action, cfg):
    DECISION_LOG.append({
        "hand_id": state.get("hand_id", len(DECISION_LOG)),
        "street": street,
        "position": position,
        "hand": hand,
        ...
        "eq": round(eq, 4) if eq is not None else None,
        "opp_id": opp_id,
        "opp_vpip": round(opp_profile.stat("vpip"), 3) if opp_profile else None,
        "opp_pfr": round(opp_profile.stat("pfr"), 3) if opp_profile else None,
        "opp_n_hands": opp_profile.hands_observed if opp_profile else 0,
        "opp_agg": round(opp_profile.aggression_factor, 3) if opp_profile else None,
        "branch": branch,
        "action": action,
        "our_match_delta": our_match_delta,
    })
    return action
...
def station_score(bot_id: str, cfg) -> float:
    ...
    if p.hands_observed > 30 and p.aggression_factor < 0.2:
        with open("station_sat.log", "a") as f:
            f.write(f"[STATION_SAT] hand={p.hands_observed} af={p.aggression_factor:.3f}\n")
        return 1.0
...
def decide_preflop_6max(state: dict, position: str, hand: str, cfg: Config,
    ...
    if not passes_variance_check(state, owed, state["your_cards"], cfg):
        if can_check:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_1", {"action": "check"}, cfg)
        return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_2", {"action": "fold"}, cfg)
    ...
    if scenario == "open":
        ...
        if eff_freq > 0 and rng.random() < eff_freq:
            limpers = sum(1 for e in log if e.get("action") == "call")
            target = int(BIG_BLIND * (cfg.open_size_bb + limpers))
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_3", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)
        if can_check:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_4", {"action": "check"}, cfg)
        return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_5", {"action": "fold"}, cfg)
    ...
    if scenario == "face_open":
        ...
        if eff_3bet > 0 and rng.random() < eff_3bet:
            ...
            mult = cfg.threebet_size_ip if ip else cfg.threebet_size_oop
            target = int(current * mult)
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_6", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)
        ...
        if opp_profile is not None:
            ...
            call_thresh *= 1.0 - cfg.k_tightness_vs_3bet_freq * max(0, tb_rate - 0.10)
        if call_freq > 0 and owed <= state["your_stack"] * call_thresh:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_7", {"action": "call"}, cfg)

        if can_check:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_8", {"action": "check"}, cfg)
        return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_9", {"action": "fold"}, cfg)
    ...
    if scenario == "face_3bet_as_raiser":
        ...
        if fourbet_risk >= cfg.fourbet_commit_threshold:
            fivebet_freq = lookup_freq(FIVEBET_FREQS, hand)
            if fivebet_freq > 0 or hand in TIGHT_MONSTERS:
                return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_10", {"action": "all_in"}, cfg)
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_11", {"action": "fold"}, cfg)

        # Shallow stack jam-or-fold
        if stack_bb < cfg.shallow_jam_threshold_bb:
            fivebet_freq = lookup_freq(FIVEBET_FREQS, hand)
            if fivebet_freq > 0 or hand in TIGHT_MONSTERS:
                return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_12", {"action": "all_in"}, cfg)
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_13", {"action": "fold"}, cfg)
        ...
        if opp_profile is not None:
            ...
            eff_4bet *= 1.0 + cfg.k_4bet_vs_3bet_freq * max(0, tb_rate - 0.10)
        if eff_4bet > 0 and rng.random() < eff_4bet:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_14", {"action": "raise", "amount": safe_raise_amount(state, proposed_4bet)}, cfg)
        ...
        if ip:
            ...
            effective_bluff_freq = cfg.fourbet_bluff_freq * (1.0 - maniac_w)
            if bluff_freq > 0 and effective_bluff_freq > 0 and rng.random() < effective_bluff_freq:
                return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_15", {"action": "raise", "amount": safe_raise_amount(state, proposed_4bet)}, cfg)
        ...
        if opp_profile is not None:
            ...
            call_thresh *= 1.0 - cfg.k_tightness_vs_3bet_freq * max(0, tb_rate - 0.10)
        if hand in {"JJ", "TT", "AKo", "AQs"} and owed <= state["your_stack"] * call_thresh:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_16", {"action": "call"}, cfg)

        if can_check:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_17", {"action": "check"}, cfg)
        return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_18", {"action": "fold"}, cfg)
    ...
    if scenario == "face_3bet_cold":
        ...
        if hand in TIGHT_MONSTERS:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_19", {"action": "all_in"}, cfg)
        call_thresh_cold = cfg.threebet_call_threshold_pct
        if opp_profile is not None:
            tb_rate = opp_profile.stat("three_bet")
            call_thresh_cold *= 1.0 - cfg.k_tightness_vs_3bet_freq * max(0, tb_rate - 0.10)
        if hand in {"JJ", "AKo", "AQs"} and owed <= state["your_stack"] * call_thresh_cold:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_20", {"action": "call"}, cfg)
        if can_check:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_21", {"action": "check"}, cfg)
        return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_22", {"action": "fold"}, cfg)
    ...
    if scenario in ("face_4bet_as_raiser", "face_5bet_as_raiser"):
        ...
        if lookup_freq(FIVEBET_FREQS, hand) > 0 or hand in PURE_MONSTERS:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_23", {"action": "all_in"}, cfg)
        if can_check:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_24", {"action": "check"}, cfg)
        return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_25", {"action": "fold"}, cfg)

    if can_check:
        return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_26", {"action": "check"}, cfg)
    return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_27", {"action": "fold"}, cfg)
...
def decide_preflop_hu(state: dict, position: str, hand: str, cfg: Config,
    ...
    if not passes_variance_check(state, state["amount_owed"], state["your_cards"], cfg):
        if can_check:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_28", {"action": "check"}, cfg)
        return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_29", {"action": "fold"}, cfg)
    aggressors = count_aggressors(state)
    stack_bb = state["your_stack"] / BIG_BLIND
    ...
    if aggressors == 0:
        if position == "BTN":
            ...
            if eff_freq > 0 and rng.random() < eff_freq:
                ...
                return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_30", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)
            if can_check:
                return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_31", {"action": "check"}, cfg)
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_32", {"action": "fold"}, cfg)
        else:
            if can_check:
                return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_33", {"action": "check"}, cfg)
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_34", {"action": "fold"}, cfg)
    ...
    if aggressors == 1:
        ...
        if eff_3bet > 0 and rng.random() < eff_3bet:
            ...
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_35", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)
        ...
        if call_freq > 0 and owed <= state["your_stack"] * cfg.threebet_call_threshold_pct:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_36", {"action": "call"}, cfg)
        if can_check:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_37", {"action": "check"}, cfg)
        return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_38", {"action": "fold"}, cfg)
    ...
    if aggressors == 2:
        ...
        if eff_4bet > 0 and rng.random() < eff_4bet:
            ...
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_39", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)
        if hand in {"QQ", "JJ", "AKo", "AQs"}:
            return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_40", {"action": "call"}, cfg)
        return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_41", {"action": "fold"}, cfg)

    if lookup_freq(HU_BB_5BET_FREQS, hand) > 0 or hand in PURE_MONSTERS:
        return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_42", {"action": "all_in"}, cfg)
    return _log_decision(state, position, hand, state.get("street", "preflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "preflop_branch_43", {"action": "fold"}, cfg)
...
def decide_river(state: dict, position: str, eq: float, opp_profile, cfg: Config, rng: random.Random) -> dict:
    ...
    hole = state.get("your_cards", state.get("hole_cards", []))
    hand = hand_str(hole)
    pot = state["pot"]
    owed = state["amount_owed"]
    can_check = state["can_check"]
    ...
    if owed > 0:
        ...
        if eq >= required_eq:
            return _log_decision(state, position, hand, state.get("street", "river"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "river_branch_44", {"action": "call"}, cfg)
        return _log_decision(state, position, hand, state.get("street", "river"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "river_branch_45", {"action": "fold"}, cfg)

    # 2. Initiating / Checking -> V2B logic
    if eq >= cfg.river_value_strong_threshold:
        target = int(pot * cfg.river_value_strong_size)
        return _log_decision(state, position, hand, state.get("street", "river"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "river_branch_46", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)
    elif eq >= cfg.river_value_thin_threshold:
        target = int(pot * cfg.river_value_thin_size)
        return _log_decision(state, position, hand, state.get("street", "river"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "river_branch_47", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)
    ...
    if eq < 0.20:
        ...
        if rng.random() < base_bluff_freq:
            target = int(pot * 0.5)
            return _log_decision(state, position, hand, state.get("street", "river"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "river_branch_48", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)

    # Middle of range -> Check
    return _log_decision(state, position, hand, state.get("street", "river"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "river_branch_49", {"action": "check"}, cfg)
...
def decide_postflop(state: dict, position: str, cfg: Config,
    ...
    if opp_id:
        station_w = station_score(opp_id, cfg)
    ...
    if can_check:
        ...
        if eq >= cfg.equity_value_bet:
            ...
            return _log_decision(state, position, hand, state.get("street", "postflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "postflop_branch_50", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)
        ...
        if was_pf_aggressor and street == "flop":
            ...
            if rng.random() < cbet_freq:
                ...
                return _log_decision(state, position, hand, state.get("street", "postflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "postflop_branch_51", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)
        ...
        if was_pf_aggressor and street in ("turn", "river") and eq < cfg.equity_thin_value:
            ...
            if barrel_freq > 0 and rng.random() < barrel_freq:
                ...
                return _log_decision(state, position, hand, state.get("street", "postflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "postflop_branch_52", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)

        # Thin value IP - not vs station with weak hand
        if eq >= cfg.equity_thin_value and in_position:
            target = int(state["current_bet"] + pot * cfg.sizing_thin)
            return _log_decision(state, position, hand, state.get("street", "postflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "postflop_branch_53", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)
        ...
        if not was_pf_aggressor and not in_position and eq > cfg.oop_passive_value_threshold:
            if opp_profile is not None and opp_profile.aggression_factor < cfg.passive_aggression_threshold:
                ...
                return _log_decision(state, position, hand, state.get("street", "postflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "postflop_branch_54", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)
        ...
        if eq < cfg.equity_thin_value and bluff_freq > 0 and rng.random() < bluff_freq:
            target = int(state["current_bet"] + pot * cfg.sizing_thin)
            return _log_decision(state, position, hand, state.get("street", "postflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "postflop_branch_55", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)

        return _log_decision(state, position, hand, state.get("street", "postflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "postflop_branch_56", {"action": "check"}, cfg)

    # === Facing a bet ===
    if owed <= 0:
        return _log_decision(state, position, hand, state.get("street", "postflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "postflop_branch_57", {"action": "check"}, cfg)
    ...
    if eq >= effective_raise_eq:
        ...
        return _log_decision(state, position, hand, state.get("street", "postflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "postflop_branch_58", {"action": "raise", "amount": safe_raise_amount(state, target)}, cfg)

    if eq >= required_eq:
        return _log_decision(state, position, hand, state.get("street", "postflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "postflop_branch_59", {"action": "call"}, cfg)
    ...
    if eq >= (cfg.equity_value_bet - cfg.k_commit * commitment_factor) and variance_term <= 0:
        return _log_decision(state, position, hand, state.get("street", "postflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "postflop_branch_60", {"action": "call"}, cfg)
    ...
    if eq >= (
        ...
        return _log_decision(state, position, hand, state.get("street", "postflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "postflop_branch_61", {"action": "call"}, cfg)

    return _log_decision(state, position, hand, state.get("street", "postflop"), locals().get("eq", None), locals().get("opp_id", None), locals().get("opp_profile", None), "postflop_branch_62", {"action": "fold"}, cfg)
...
def decide(game_state: dict) -> dict:
    ...
    try:
        ...
        import traceback
        with open("diag_exceptions.log", "a") as f:
            f.write("CAUGHT SILENT EXCEPTION 1:\n" + traceback.format_exc() + "\n")

    position = get_position_label(game_state)
    hand = hand_str(game_state.get("your_cards", game_state.get("hole_cards", [])))
